...
import { buildAuthPayload, getConfigWithSecurity, maskSecurity, requireAdmin } from '../security/auth.js';
...
router.get('/config', async (req, res) => {
  try {
    ...
    res.json({ ...maskSecurity(cfg), auth: buildAuthPayload(req, cfg) });
  } catch (err) {
...
router.put('/config', requireAdmin, async (req, res) => {
  try {
    ...
    res.json({ ...maskSecurity(merged), auth: buildAuthPayload(req, merged) });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
