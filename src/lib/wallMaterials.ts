...
/** Resolve exterior/interior colors for a wall segment */
export function resolveWallColors(wall: {
  ...
  return {
    exteriorColor: extMat?.color ?? '#e8a845',
    interiorColor: intMat?.color ?? extMat?.color ?? '#e8a845',
    edgeColor: extMat?.color ?? '#e8a845',
    exteriorRoughness: extMat?.roughness ?? 0.8,
    interiorRoughness: intMat?.roughness ?? extMat?.roughness ?? 0.8,
  };
}
