  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <p className="text-2xl font-bold font-display text-foreground tracking-tight">{time}</p>
      </div>
      {expanded ? (
        <div className="mt-2 pt-2 border-t border-border/30 space-y-1">
          <p className="text-xs text-foreground capitalize">{date}</p>
          <p className="text-[10px] text-muted-foreground">Sekunder: :{seconds}</p>
          <p className="text-[10px] text-muted-foreground">Tidszon: {timezone}</p>
          <p className="text-[10px] text-muted-foreground">Vecka {getWeekNumber(now)}</p>
        </div>
      ) : (
        <p className="text-[10px] text-muted-foreground capitalize mt-0.5 truncate">{date}</p>
      )}
    </div>
  );
}
