...
const storeCreator = (set: any, get: any): AppState => ({
  ...
  energyConfig: { pricePerKwh: 1.5, currency: 'kr', dailyGoalKwh: 30 },
  calendar: {
  ...
  dashboard: { activeCategory: 'home' as const, categoryLayouts: {}, density: 'balance' as const, editMode: false },
  ...
  setDashboardDensity: (density) => { set((s: any) => ({ dashboard: { ...s.dashboard, density } })); syncProfileToServer(); },
  toggleDashboardEditMode: () => { set((s: any) => ({ dashboard: { ...s.dashboard, editMode: !s.dashboard.editMode } })); },
  ...
  toggleComfortRule: (id) => { set((s: any) => ({ comfort: { ...s.comfort, rules: s.comfort.rules.map((r: any) => r.id === id ? { ...r, enabled: !r.enabled } : r) } })); syncProfileToServer(); },
  setComfortOverride: (override) => { set((s: any) => ({ comfort: { ...s.comfort, override: { ...s.comfort.override, ...override } } })); syncProfileToServer(); },
  ...
  environment: {
    ...
    sunCalibration: { northOffset: 0, azimuthCorrection: 0, elevationCorrection: 0, intensityMultiplier: 1.0, indoorBounce: 1.0 },
  ...
  setRoomMaterial: (floorId, roomId, target, materialId) =>
    set((s: any) => {
      ...
      // Wall material: set wallMaterialId on the room object (rendered by RoomWallSurfaces3D)
      return {
        layout: {
          ...s.layout,
          floors: s.layout.floors.map((f: any) =>
